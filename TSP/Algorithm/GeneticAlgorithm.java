...
public class GeneticAlgorithm {
    ...
    public Chromosome run(int loop, Crossover cro, Mutation mut, Selection sel, Random r) {
        ...
        while (i++ < loop) {
            ...
            System.out.println("Generation " + i + " :" + problem.fitness(maxChromosome(population)));
        }

        return maxChromosome(population);
    }

    private Chromosome maxChromosome(Population p) {
        return Collections.min(population.getValuePopulation(), new Comparator<Chromosome>() {
            @Override
